...
use examples::fluid::{
    ADVECT_RADIUS, DEFAULT_GRID_SIZE, DIFF, DIFFUSION_ITERS, DT, FluidState, PROJECT_ITERS,
    STIR_DYE_STRENGTH, STIR_RADIUS, STIR_STRENGTH, VISC, flatten_inputs, initial_state,
    simulate_ops,
};
use examples::r1cs::{ConstraintGenerator, R1CSExec, WitnessGenerator, check_constraints};
use ndarray::{Array, IxDyn};

const EPSILON: f64 = 1e-8;
const N: usize = DEFAULT_GRID_SIZE;
...
// Derived from fluid.py.
fn known_answer_dye() -> Array<f64, IxDyn> {
    let values = [
        [100.0, 100.0, 100.0, 100.0, 100.0, 100.0, 100.0, 100.0],
        [100.0, 98.40539417, 100.0, 100.0, 100.0, 100.0, 100.0, 100.0],
        [
            100.0,
            100.0,
            100.0,
    ...
    Array::from_shape_vec(IxDyn(&[N, N]), values.into_iter().flatten().collect())
...
fn eval_array(
    symbolic: &Array<examples::builder::LC, IxDyn>,
    witness: &[f64],
) -> Array<f64, IxDyn> {
...
#[test]
fn step_consistency() {
    let mut expected = initial_state(N);
    ...
    let symbolic = simulate_ops(&mut cons_gen, 1, N);
...
fn known_answer_test() {
    ...
    let mut exp_gen = WitnessGenerator::new_from_tensored_inputs(flatten_inputs(&initial_state(N)));
    let actual = simulate_ops(&mut exp_gen, 1, N);
